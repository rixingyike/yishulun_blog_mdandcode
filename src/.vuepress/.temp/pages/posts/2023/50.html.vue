...
<p><img src="@source/posts/2023/assets/v2-bb988e6b4df313a9e205385a5f359467_720w.png" alt="img"></p>
...
<p><img src="@source/posts/2023/assets/v2-b7b828e6bba21f8799e73b3b3b409f64_720w.png" alt="img"></p>
<p>如果不总是挑选 “顶级” 词，而是有时随机挑选 “非顶级” 词（“随机性” 对应 “温度” 为 0.8）呢？人们又可以建立起看起来较为合理的文本。而每次这样做，都会有不同的随机选择，文本也会不同。</p>
<p><img src="@source/posts/2023/assets/v2-f3097707117b861cf26fe24f198a0569_720w.png" alt="img"></p>
...
<p><img src="@source/posts/2023/assets/v2-e4b7769ea2c2e245f3aacf48adb9887a_720w.png" alt="img"></p>
<p>而这对狗的文章，也有同样的作用：</p>
<p><img src="@source/posts/2023/assets/v2-54b14a0a46d8adc33f655085cfcf5257_720w.png" alt="img"></p>
...
<p><img src="@source/posts/2023/assets/v2-5bfcff014891bca1f0f686397b642f9c_720w.png" alt="img"></p>
<p>显然，这是一派胡言。</p>
<p>那么，我们如何才能做得更好呢？就像对待字母一样，我们可以开始考虑的不仅仅是单个词的概率，还有成对的或更长的词的 n-grams 的概率。在成对的情况下，以下是我们得到的 5 个例子，所有情况都是从 “cat” 这个词开始的：</p>
<p><img src="@source/posts/2023/assets/v2-db876030852bbf72cac2022deb6b009a_720w.png" alt="img"></p>
...
<p>拼写纠错：可以使用 bigram 来判断两个单词是否可以连续出现。例如，如果 &quot;is&quot; 和 &quot;were&quot; 的 bigram 频率很高，那么我们可以认为 &quot;is&quot; 后面跟着 &quot;were&quot; 是比较合理的。
...
<p><img src="@source/posts/2023/assets/v2-2239c4bf74cf3f2c3011e09ae0339a8b_720w.png" alt="img"></p>
...
<p><img src="@source/posts/2023/assets/v2-f5327c5df18eaa5abd3fec3ca8afa2fe_720w.png" alt="img"></p>
...
<p><img src="@source/posts/2023/assets/v2-073740d975a7e3508f33c403bc53d3d9_720w.png" alt="img"></p>
<p>不过，事情可能会出大问题。比如这里是我们用 a + b/c + x sin(x) 最多也就做成：</p>
<p><img src="@source/posts/2023/assets/v2-ba5dafa449840d7a4afc8a3d69ea1f14_720w.png" alt="img"></p>
...
<p><img src="@source/posts/2023/assets/v2-4b73bdbb0e78fd9ae70e04b8958d9112_720w.png" alt="img"></p>
<p>我们可以做的一件事是为每个数字获取一堆样本图像：</p>
<p><img src="@source/posts/2023/assets/v2-664efd29d99b4b862109f2b2594e5424_720w.png" alt="img"></p>
<p>然后，为了找出我们输入的图像是否对应于某个特定的数字，我们只需与我们拥有的样本进行明确的逐像素比较。但作为人类，我们似乎可以做得更好 —— 因为我们仍然可以识别数字，即使它们是手写的，并且有各种各样的修改和扭曲。</p>
<p><img src="@source/posts/2023/assets/v2-9c980ab1a940cb32c92b0dd8e0a73edd_720w.png" alt="img"></p>
...
<p><img src="@source/posts/2023/assets/v2-9713c70062a8f4cbca11afdd4c6c5bc0_720w.png" alt="img"></p>
<p>但这里到底发生了什么？比方说，我们逐步模糊一个数字。有一段时间，我们的函数仍然 “识别” 它，在这里是一个 “2”。但很快它就 “失去” 了，并开始给出 “错误” 的结果：</p>
<p><img src="@source/posts/2023/assets/v2-9ffc2948328e8ab441e4f5293c02b6f4_720w.png" alt="img"></p>
...
<p><img src="@source/posts/2023/assets/v2-b36ec74cc32b664285f293d6f90baa48_720w.png" alt="img"></p>
...
<p><img src="@source/posts/2023/assets/v2-893ea8ddf9d5091c80a840f1cca90134_720w.png" alt="img"></p>
...
<p><img src="@source/posts/2023/assets/v2-f69050e4aec578dd068a6f5870539dd7_720w.png" alt="img"></p>
...
<p><img src="@source/posts/2023/assets/v2-85c1bfcf8a1c04621ed9645459f20c13_720w.png" alt="img"></p>
<p>我们的目标是获取一个对应于 {x,y} 位置的 “输入”，然后将其 “识别” 为它最接近的三个点中的任何一个。或者，换句话说，我们希望神经网络能够计算出一个类似于 {x,y} 的函数：</p>
<p><img src="@source/posts/2023/assets/v2-bfe865d56a5958e8b3e1c166e4a5aa1b_720w.png" alt="img"></p>
<p>那么，我们如何用神经网络做到这一点呢？归根结底，神经网是一个理想化的 “神经元” 的连接集合 —— 通常按层排列 —— 一个简单的例子是：</p>
<p><img src="@source/posts/2023/assets/v2-2b546bd41b67ffe3e0950f6e4c7f163e_720w.png" alt="img"></p>
<p>每个 “神经元” 都被有效地设置为评估一个简单的数字函数。为了 “使用” 这个网络，我们只需在顶部输入数字（如我们的坐标 x 和 y），然后让每一层的神经元 “评估它们的功能”，并通过网络向前输入结果 —— 最终在底部产生最终的结果。</p>
<p><img src="@source/posts/2023/assets/v2-e12e179f97abd6168841d5331d5af5bf_720w.png" alt="img"></p>
...
<p><img src="@source/posts/2023/assets/v2-a1a2be4855b84470c4cef5e2fb5a8ee9_720w.png" alt="img"></p>
...
<p><img src="@source/posts/2023/assets/v2-00de60e61142cec3471d812574053eee_720w.png" alt="img"></p>
...
<p><img src="@source/posts/2023/assets/v2-0e759b98786aeddc7616ef8859827a7c_720w.png" alt="img"></p>
<p>但是，上面那个更大的网络是怎么回事？嗯，这是它的计算结果：</p>
<p><img src="@source/posts/2023/assets/v2-70d12133c352fab76ffbf23be3a7890c_720w.png" alt="img"></p>
<p>这不是很 “正确”，但它接近于我们上面展示的 “最近点” 函数。</p>
<p>让我们看看其他一些神经网络的情况。在每一种情况下，正如我们稍后所解释的，我们都在使用机器学习来寻找最佳的权重选择。然后，我们在这里展示带有这些权重的神经网络的计算结果：</p>
<p><img src="@source/posts/2023/assets/v2-d4d246e46bdfad157e3b15de5da46171_720w.png" alt="img"></p>
...
<p><img src="@source/posts/2023/assets/v2-128a6aa4df654504bca72e1d3e83f941_720w.png" alt="img"></p>
...
<p><img src="@source/posts/2023/assets/v2-993f4d0ccd3ce15fa91f7315aaf7489c_720w.png" alt="img"></p>
...
<p>如果我们采取一个典型的猫图像：</p>
<p><img src="@source/posts/2023/assets/v2-c392c0984daa9793555d9a92b343d450_720w.png" alt="img"></p>
<p>那么我们就可以用一组衍生图像来表示第一层神经元的状态 —— 其中许多图像我们可以很容易地解释为 “没有背景的猫” 或 “猫的轮廓” 等：</p>
<p><img src="@source/posts/2023/assets/v2-5bd1f274deacb1b62df9661959ce4b91_720w.png" alt="img"></p>
<p>到了第十层，就更难解释发生了什么：</p>
<p><img src="@source/posts/2023/assets/v2-b577b51e53a2a2fe3702223ad36a36ed_720w.png" alt="img"></p>
...
<p>参考</p>
<ul>
<li>https://zhuanlan.zhihu.com/p/631347310</li>
</ul>
</div></template>
