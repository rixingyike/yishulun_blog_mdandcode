...
fn main() {
    ...
    (&mut (*wrapper)).foo(3); // 输出 3 MyStruct::foo called by reference
...
第45行，这是一个老老实实的值方法调用。第46行调用不了，那是因为在第45行value的所有权被转移，被消耗了。第50行和第54行的调用是老老实实的调用，代码中的对象写法，与方法中的参数类型是相同的。然而第51行和第55行的写法，就应用了点操作符的自动转换规则，这种写法很简洁，但不了解的程序员看了会感到迷惑。有时候初学者反而不容易感到迷惑，反而是有经验的程序员更容易感到迷惑。
