<template>
  <div class="authorCard">
    <h1 class="title">
      <span>Book</span>
    </h1>
    <p class="description">
      <a href="https://item.jd.com/13728755.html" target="_blank"><img style="max-width: 300px;" src="/wegame-book.png" /></a>
    </p>

    <h1 class="title">
      <span>Courses</span>
    </h1>
    <p class="description">
      <a href="http://gk.link/a/10AdC" target="_blank"><img style="max-width: 400px" src="/geektime-weapp.png" /></a>
    </p>
    <p class="description">
      <a href="http://gk.link/a/10AdC" target="_blank"><img style="max-width: 400px" src="/weapp-optimize.png" /></a>
    </p>

    <h1 class="title">
      <span>Links</span>
    </h1>
    <p class="description">
      <a href="/posts/2023/02.html" target="_blank">《微信小游戏开发》勘误列表</a>
    </p>
  </div>
</template>
<style scoped>
.authorCard {
  display: flex;
  flex-direction: column;
  align-items: center;
  padding: 60px 0 15px;
  text-align: center;
}

.title {
  display: flex;
  align-items: center;
  font-size: 2.2em;
  font-weight: bold;
  margin-top: 30px;
}

.description {
  margin: 1em 0;
}
</style>
