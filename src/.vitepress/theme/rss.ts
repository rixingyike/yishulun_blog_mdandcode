...
const AUTHOR = {
  name: "LIYI",
  email: "coderliyi@gmail.com",
  link: DOMAIN,
};
const OPTIONS: FeedOptions = {
  title: "LIYI",
  description: "LIYI' Blog",
  id: `${DOMAIN}/`,
  link: `${DOMAIN}/`,
...
async function writeFeed(name: string, items: Item[]) {
  ...
  await fs.ensureDir(dirname(`./src/.vitepress/dist/${name}`));
  await fs.writeFile(`./src/.vitepress/dist/${name}.xml`, feed.rss2(), "utf-8");
  await fs.writeFile(`./src/.vitepress/dist/${name}.atom`, feed.atom1(), "utf-8");
  await fs.writeFile(`./src/.vitepress/dist/${name}.json`, feed.json1(), "utf-8");
}
